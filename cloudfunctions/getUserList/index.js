...
exports.main = async (event, context) => {
  ...
  try {
    ...
    // 使用聚合查询：关联 freeBuy 集合获取免单次数，关联user集合获取邀请人信息
    ...
    const list = (aggregateRes.list || []).map(user => {
      ...
      // 提取邀请人信息
      let inviterNickName = ''
      if (user.inviterInfo && Array.isArray(user.inviterInfo) && user.inviterInfo.length > 0) {
        inviterNickName = user.inviterInfo[0].nickName || ''
      }

      // 移除不需要的字段
      const { freeBuyInfo, inviterInfo, ...userData } = user
      return {
        ...userData,
        miandanCount: miandanCount,
        inviterNickName: inviterNickName,
        inviteCode: user.inviteCode || '', // 确保返回邀请码
        inviteCount: user.inviteCount || 0 // 确保返回邀请人数，默认为0
      }
    })
