...
Page({
  ...
  async loadGoods(menuId, append = false, showLoading = true) {
    ...
    try {
      ...
      // 为每个菜品添加购物车数量，并转换图片URL为可访问的HTTP地址
      const list = goodsRes.data || []
      const imageUrls = list.map(goods => goods.image).filter(url => url && url.startsWith('cloud://'))
      let urlMap = {}
      if (imageUrls.length > 0) {
        const tempUrlRes = await wx.cloud.getTempFileURL({ fileList: imageUrls })
      ...
      const mapped = list.map(goods => {
        ...
        if (goods.image && urlMap[goods.image]) {
          goods.image = urlMap[goods.image]
        }
        return goods
      })
