...
Page({
  data: {
    ...
    totalPrice: 0, // 商品总价
    finalPrice: 0, // 实付金额
    orderType: 'dineIn', // 订单类型：dineIn-宿舍，takeOut-打包
  ...
  loadCartData() {
    try {
      ...
      if (cartData) {
        ...
        // 根据是否有邀请码设置默认订单类型（有邀请码默认宿舍，无桌码默认打包）
        // 但由于现在都需要邀请码，所以默认设置为宿舍
        // 确保价格是数字类型
  ...
  async submitOrder(e) {
    ...
    try {
      ...
      const outTradeNo = doBuyRes.result.outTradeNo // ✅ 修改1：获取后端返回的outTradeNo
      ...
      if (payWithBalance || useMiandan) {
        ...
        const payRes = await wx.cloud.callContainer({
          config: {
          ...
          data: {
            ...
            openid: openid,
            outTradeNo: outTradeNo // ✅ 修改2：使用后端返回的outTradeNo，而不是orderId
          }
        })

        console.log('支付云托管API完整返回:', JSON.stringify(payRes, null, 2))

        // 检查返回结果
        if (!payRes.data) {
          throw new Error('API未返回结果')
        }

        // 检查是否成功
        if (!payRes.data.success) {
          const errorMsg = payRes.data.error || payRes.data.message || '支付失败'
        ...
        setTimeout(() => {
          wx.switchTab({
            ...
            success: function() {
              ...
              const myorderPage = pages[pages.length - 1]
              if (myorderPage && myorderPage.route === 'pages/myorder/myorder') {
                // 重置分页状态
                myorderPage.setData({
                  orderPage: 0,
                  orderHasMore: true,
